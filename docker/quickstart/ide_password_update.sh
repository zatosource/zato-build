...
if [[ -n "${ZATO_IDE_PUBLISHER_PASSWORD}" ]]; then
    ...
    user_id="$(curl -q "http://admin:$(cat ~/web_admin_password)@localhost:11223/zato/api/invoke/zato.security.basic-auth.get-list?cluster_id=1" 2>/dev/null | ~/jq '.[] | select(.username == "ide_publisher").id')"
