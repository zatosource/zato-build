...
function build_deb {
    ...
    [[ -d "/tmp/zato-build/$(lsb_release -c | cut -f2)/" ]] || mkdir -p "/tmp/zato-build/$(lsb_release -c | cut -f2)/"
    cp "$CURDIR/zato-$ZATO_VERSION-$PACKAGE_VERSION\_$ARCH-$RELEASE_NAME.deb" "/tmp/zato-build/$(lsb_release -c | cut -f2)/"
}
